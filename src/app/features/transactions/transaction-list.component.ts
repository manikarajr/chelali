  ...
  template: `
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Transactions</h2>
          <p class="text-sm text-gray-500 mt-0.5 sm:mt-1">Ice purchase records</p>
        </div>
        <button
          (click)="openAdd()"
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs tracking-wide">
              <tr>
                ...
                <th class="px-4 py-3 text-right font-semibold hidden md:table-cell">Qty (kg)</th>
                <th class="px-4 py-3 text-right font-semibold hidden md:table-cell">Unit Price</th>
                <th class="px-4 py-3 text-right font-semibold">Total</th>
                <th class="px-4 py-3 text-right font-semibold hidden sm:table-cell">Paid</th>
            ...
            <tbody class="divide-y divide-gray-100">
              @for (t of filtered(); track t.id) {
                <tr class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-4 py-3 text-gray-500 hidden sm:table-cell">{{ t.date | date:'dd MMM yyyy' }}</td>
                  <td class="px-4 py-3 text-right text-gray-700 hidden md:table-cell">{{ t.quantity }}</td>
                  <td class="px-4 py-3 text-right text-gray-700 hidden md:table-cell">
                    {{ t.unitPrice | currency:'INR':'symbol':'1.0-0' }}
                  </td>
                  ...
                  <td class="px-4 py-3 text-right font-semibold"
                      [ngClass]="t.outstandingAmount > 0 ? 'text-red-600' : 'text-gray-400'">
                    {{ t.outstandingAmount | currency:'INR':'symbol':'1.0-0' }}
                  </td>
                ...
                <tr>
                  <td colspan="9" class="px-4 py-12 text-center text-gray-400">No transactions found.</td>
                </tr>
              }
            </tbody>
          </table>
