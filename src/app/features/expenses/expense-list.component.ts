    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-0.5 sm:mt-1">Track operational expenses</p>
        </div>
        <button
          (click)="openAdd()"
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        @for (cat of categorySummary(); track cat.label) {
          <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <p class="text-xs text-gray-500 mb-1">{{ cat.label }}</p>
            <p class="text-lg font-bold text-gray-900">{{ cat.total | currency:'INR':'symbol':'1.0-0' }}</p>
          </div>
        }
      </div>
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs tracking-wide">
              <tr>
                ...
                <th class="px-4 py-3 text-left font-semibold hidden md:table-cell">Notes</th>
            ...
            <tbody class="divide-y divide-gray-100">
              @for (e of filtered(); track e.id) {
                <tr class="hover:bg-gray-50 transition-colors">
                  <td class="px-4 py-3">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                          [ngClass]="categoryColor(e.category)">
                      {{ e.category | titlecase }}
                    </span>
                  </td>
                  <td class="px-4 py-3 text-gray-500 hidden sm:table-cell">{{ e.date | date:'dd MMM yyyy' }}</td>
                ...
                <tr>
                  <td colspan="5" class="px-4 py-12 text-center text-gray-400">No expenses found.</td>
                </tr>
              }
            </tbody>
          </table>
